<template>
  <div>
    ...
    <div v-if="response && onHover">
        <a href="javascript:;" @mouseover="hoverIn" @click="viewMore">{{ linkTitle }}</a>
        <div v-if="showPopup" @mouseleave="hoverOut">
            <div class="user-popover" transition="fade" :style="{width:cardWidth}">
            <slot :img="response.images[0]" :title="response.title" :description="response.description" :url="url">
<!--                 
                <h3>{{response.title}}</h3>
                <p>{{response.description}}</p>
                <button @click="viewMore">Follow</button> -->
                <div class="wrapper" :style="{width:cardWidth}">
                    <div class="card-img">
                        <img :src="response.images[0]">
                        </div>
